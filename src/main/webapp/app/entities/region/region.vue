<template>
  <div>
    <h2 id="page-heading" data-cy="RegionHeading">
      <span v-text="$t('appdemoApp.region.home.title')" id="region-heading">Regions</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('appdemoApp.region.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'RegionCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('appdemoApp.region.home.createLabel')"> Create a new Region </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && regions && regions.length === 0">
      <span v-text="$t('appdemoApp.region.home.notFound')">No regions found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="regions">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('appdemoApp.region.regionName')">Region Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="appdemoApp.region.delete.question" data-cy="regionDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-region-heading" v-text="$t('appdemoApp.region.delete.question', { id: removeId })">
